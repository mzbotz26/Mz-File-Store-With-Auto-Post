...
from from plugins.web_server import web_server
...
class Bot(Client):
    ...
    async def _setup_force_sub(self, channel, attr):
        if not channel:
            return
        ...
        try:
            ...
            self.LOGGER(__name__).warning(e)
            self.LOGGER(__name__).warning(f"Bot can't export invite link from {channel}")
            sys.exit()
